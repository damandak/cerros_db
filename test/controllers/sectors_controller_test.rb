...
class SectorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create sector" do
    assert_difference('Sector.count') do
      post sectors_url, params: { sector: { category: @sector.category, name: @sector.name } }
    end
  ...
  test "should update sector" do
    patch sector_url(@sector), params: { sector: { category: @sector.category, name: @sector.name } }
